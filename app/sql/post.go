...
var (
	...
	GetPostById = `SELECT id, author, message, thread, parent, forum, isedited
				   FROM posts
				   WHERE id = $1;`
	...
	UpdatePostById = `UPDATE posts
					  ...
					  WHERE id = $1
					  RETURNING id, author, message, thread, parent, forum, isedited;`

	// Flat sort
	GetPostsByIdFlatSinceDesc = `SELECT id, author, message, parent
								 FROM posts
								 WHERE thread = $1 AND id < $2::TEXT::INTEGER
								 ORDER BY id DESC
								 LIMIT $3::TEXT::INTEGER;`

	GetPostsByIdFlatSince = `SELECT id, author, message, parent
							 FROM posts
							 WHERE thread = $1 AND id > $2::TEXT::INTEGER
							 ORDER BY id
							 LIMIT $3::TEXT::INTEGER;`

	GetPostsByIdFlatDesc = `SELECT id, author, message,  parent
							FROM posts
							WHERE thread = $1
							ORDER BY id DESC
							LIMIT $2::TEXT::INTEGER;`

	GetPostsByIdFlat = `SELECT id, author, message, parent
						FROM posts
						WHERE thread = $1
						ORDER BY id
						LIMIT $2::TEXT::INTEGER;`
	...
	// Tree sort
	GetPostsByIdTreeSinceDesc = `SELECT id, author, message, parent 
								 FROM posts 
	...
	GetPostsByIdTreeSince = `SELECT id, author, message, parent 
							 FROM posts 
							 WHERE thread = $1 
							 AND path > 
							 	(
	...
	GetPostsByIdTreeDesc = `SELECT id, author, message, parent 
							FROM posts 
							WHERE thread = $1 
							ORDER BY path DESC 
							LIMIT $2::TEXT::INTEGER;`

	GetPostsByIdTree = `SELECT id, author, message, parent 
						FROM posts 
						WHERE thread = $1 
						ORDER BY path 
						LIMIT $2::TEXT::INTEGER;`
	...
	GetPostsByIdParentTreeDesc = `SELECT p.id, p.author, p.message, p.parent 
								  FROM posts p
								  JOIN (
	...
	GetPostsByIdParentTree = `SELECT p.id, p.author, p.message, p.parent 
							  FROM posts p
							  JOIN (
								  SELECT id as rootParentsId
								  FROM posts 
